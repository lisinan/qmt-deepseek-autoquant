...
def main() -> int:
    print("=" * 60)
    print("qmtIDE-deepseek DeepSeek API 自检")
    print("=" * 60)

    print(f"\n[配置] API key 配置: {'是' if deepseek.enabled else '否'}")
    print(f"[配置] 模型: {deepseek.model}")
    print(f"[配置] 端点: {deepseek.base_url}")
    ...
    if resp:
        ...
        print("  ❌ 失败（key 无效/网络/超时）")
        return 1

    # 2) JSON 模式
    print("\n[2/3] JSON 模式...")
    parsed = deepseek.chat_json(
        messages=[
            {"role": "system",
    ...
    # 3) AIAnalyst 集成
    print("\n[3/3] AIAnalyst.analyze() 集成...")
    print(f"  ai_analyst.client = {type(ai_analyst.client).__name__}")
    print(f"  ai_analyst.enabled = {ai_analyst.enabled}")
    ind = {
