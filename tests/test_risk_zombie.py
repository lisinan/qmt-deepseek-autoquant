...
def _mk(consec: int, loss_date: date | None, daily_pnl: float = 0.0) -> RiskManager:
    r = RiskManager()
    r._consec_loss = consec
    r._halted = False
    r._halt_reason = ""
    r._consec_loss_date = loss_date      # 连亏起算日（已持久化）
...
def test_zombie_freeze_self_heals_after_cooldown():
    ...
    r = _mk(consec=9, loss_date=date.today() - timedelta(days=5),
            daily_pnl=-188766.82)
...
def test_zombie_freeze_stays_frozen_before_cooldown():
    """冷却未满（连亏起算日就是今天）→ 不解封，避免刚冻结就立刻放行。"""
    r = _mk(consec=9, loss_date=date.today())
    r.on_asset_update(803680.18, day_open_asset=803680.18)
    assert r.position_scale == 0.0, "冷却未满不应解封"
    assert r.consecutive_losses == 9


def test_legacy_zombie_without_timestamp_heals_immediately():
    """★ 老库升级场景：consec_loss>=阈值 但无「连亏起算日」。

    无法证明这段连亏是今天发生的，而它确实跨多个交易日滞留在 engine_state
    → 必须立即解封，否则账户永远开不了仓（首版修复用 _halt_day 判定的缺陷）。
    """
    r = _mk(consec=9, loss_date=None, daily_pnl=-188766.82)
    assert r.position_scale == 0.0
    r.on_asset_update(803680.18, day_open_asset=803680.18)
    assert r.position_scale == 1.0, "无时间戳的历史僵尸态应立即解封"
    assert r.consecutive_losses == 0


def test_risk_state_persist_roundtrip():
    """连亏起算日与 halt 状态必须能被持久化并在恢复后还原。"""
    r = _mk(consec=9, loss_date=date.today() - timedelta(days=5))
    r._halted = True
    r._halt_reason = "consec_loss=9"
    blob = r.export_state()
    assert blob["consec_loss_date"] == (date.today() - timedelta(days=5)).isoformat()
    r2 = RiskManager()
    r2.load_state(blob)
    assert r2._consec_loss_date == date.today() - timedelta(days=5)
    assert r2.is_halted is True
    # 恢复自 DB 的陈旧连亏后（无 halt）应能自愈
    r3 = _mk(consec=9, loss_date=date.today() - timedelta(days=5))
    r3.load_state(r3.export_state())
    r3.on_asset_update(803680.18, day_open_asset=803680.18)
    assert r3.position_scale == 1.0


def test_zombie_heal_preserves_drawdown_baseline():
    """自愈只清连亏/日内盈亏，**保留 peak_asset**，回撤熔断不被架空。"""
    r = _mk(consec=9, loss_date=date.today() - timedelta(days=3))
...
def test_daily_pnl_resets_on_day_rollover_without_fills():
    ...
    r = _mk(consec=0, loss_date=None, daily_pnl=-5000.0)
