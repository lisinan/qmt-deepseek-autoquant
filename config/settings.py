...
STRATEGY_PARAMS = {
    ...
    # ---- 北向资金协同闸门（全新正交轴，2026-09-19 周度研究落盘）----
    # 与价格动量正交（同期相关 r≈0.09）、与 regime 独立生效。滚动 nb_lookback 日
    # 累计净买入<0 → 不开新仓（已有持仓照常离场），对冲外资系统性撤离/隔夜跳空。
    # 证据（strategy/_research_northbound_wf.py，25 只 AI 宇宙 × 713 根日线）：
    #   IS Sharpe 1.52→1.65、全样本 alpha −111→−90pt；OOS 4 窗口均值 dSh +0.101
    #   （过 +0.10 闸门）；MDD 改善、worst 全 >−15%。边界值(+0.101)，可逆。
    # 热读：改回 "off" 即完全恢复原策略行为，零代码改动；nb 序列引擎启动时预热。
    "northbound_mode": "gate",
    "nb_lookback": 20,

    # ---- 观察篮（manual entry）【2026-09-18 新增，仅用于 paper 评估】----
    # 背景：2026-09-16 强平后宇宙内 12 只全部 trend_up=False，日线闸门整体关闭，
    #   paper 账户连续两日 100% 现金、零成交 —— 无持仓可评估，自进化闭环失去度量对象。
    #   用户明确要求「买入 LLM 排序前五，有持仓才能更好评估」（且明确为 paper 单）。
    # 机制：manual_entry_codes 里的代码**绕过**动量闸门 / 日线闸门 / 评分阈值建仓，
    #   用于把「大模型排序结果」这种**尚未纳入策略**的候选真正拿在手里观察。
    # 不绕过什么（重要）：仍完整走 _handle_buy —— 风控熔断、日内次数上限、
    #   现金夹紧、max_positions、max_position_amount 全部照旧生效，只是不卡信号闸门。
    # 清单**每次热读**（STRATEGY_PARAMS.get），改清单无需重启引擎。
    # 可逆：置回空列表即完全恢复原策略行为，不动任何已验证参数。
    "manual_entry_codes": [
        "300502.SZ", "300308.SZ", "002415.SZ", "000977.SZ", "603986.SH",
    ],  # 2026-09-18 14:43 LLM 重排序前五（paper 观察）
    # 观察篮仓位是否豁免「趋势破位 / 超时 / 单日暴跌」退出，仅保留 -18% 硬止损。
    # 必须开启：这 5 只当前都在 MA60 下方，若照常跑 on_exit，建仓后下一轮就会被
    #   「趋势破位离场」立刻卖出 → 买入/卖出空转，等于没建仓。
    ...
    "risk_per_trade": 0.012,        # 单笔风险占总资产比例（2026-09-16 由 0.02 收紧：实盘 -19.6% 后降敞口去风险；真实单笔风险随之由 ~5.3% 降至 ~3.2%）
    "max_position_amount": 180000,  # 单标的最大仓位(元)（2026-09-16 由 30万收紧去风险）
    ...
    "max_positions": 5,
    # 板块轮动 / 弱换强（2026-09-07 新增，已开启）：
    #   根因：策略「集中最强5只+趋势骑行+仅止损/破位退出」在满仓时无法参与新热板块
    #   （如 09-07 光模块踏空：5 槽被非光模块水下老仓占满，51% 现金干躺）。
    #   初版机制完全依赖**日线**信号（动量前 N + on_daily_features 评分差），但日线在
    #   开盘跳空/盘中突破时严重滞后——光模块开盘拉升，日线评分与 60 日动量要等收盘才
    #   更新，于是「热板块候选明明在涨、AI 观察层也 bullish，轮动却因日线评分差不够而
    #   迟迟不换」。2026-09-07 盘后修正为**日内信号驱动**（见下 3 个新旋钮）。
    #   ★ 用户已确认开启（enable_rotation=True）。以下为轮动全部旋钮：
    "enable_rotation": True,
    "rotation_min_score_gap": 2.0,   # 非突破候选：候选日线评分 − 最弱持仓 ≥ 此值才换
    "rotation_cooldown_sec": 1800,   # 两次轮动最小间隔（秒），防抖动/防反复被割
    "rotation_hot_top_n": 15,        # 取板块推荐池/LLM top 前 N 作为「热板块」候选源
    # —— 日内信号驱动修正（解决开盘踏空）——
    "rotation_intraday_breakout_pct": 1.5,  # 候选当日涨幅≥此值视为「日内突破」（用 tick.change_pct）
    "rotation_min_score_gap_hot": 0.0,      # 突破候选放宽评分差门槛（AI热度+日内突破替代滞后日线分）
    "rotation_max_swaps_per_eval": 3,       # 单次评估最多换几只（捕捉多只联动板块，如光模块3只）
    # —— 4/5 补强空槽（2026-09-12 新增，用户确认开启）——
    "rotation_fill_empty_slot": True,       # 组合差一仓(4/5)时直接把空槽补上合格热板块候选（纯买入、不卖 existing）

    # ---- 轮动是否须通过与主信号路径同一个日线闸门【2026-09-23 AM-EVOLVE】----
    # ★ 当前 = False（**保持既有实盘行为不变**）。本轮已定位一个真实的口径背离并把
    #   修复实现完毕、回测验证完毕，但**未达晋升闸门 ②**，故按纪律不启用。置 True 即启用。
    #
    # 【背离本体】_maybe_rotate 的两条建仓分支（补强空槽 / 弱换强）原本：
    #     is_breakout = tick.change_pct >= rotation_intraday_breakout_pct(1.5)
    #     if not is_breakout and sig.side != "BUY": continue
    #   ⇒ 只要日内涨幅达标，即使 on_daily_features 因日线闸门返回 HOLD
    #     （reason="daily-gate:bias=... trend_up=False"）也照样买入。
    #   即轮动**绕过了 09-22 刚落盘、且回测已验证为正向的 min_daily_bias 闸门**，
    #   而回测器 backtest_daily.py 中**根本没有 rotation 逻辑**（grep 零命中）
    #   ⇒ 典型「实盘有、回测无」分支，违反 §7 口径纪律。
    # 【实盘证据】2026-09-23 10:00:52 以「daily-gate:bias=-0.30 trend_up=False」
    #   买入 688008.SH（板块轮动补强空槽），3 秒后即被「趋势破位离场」判定卖出
    #   （T+1 锁住），次日开盘真的卖掉 → 买/卖空转、双边 0.15%×2 摩擦。
    # 【回测证据】在 backtest_daily 新增代理字段 breakout_bypass_gate
    #   （默认 99.0=关闭，零行为变化）后测得 90×6 OOS 单调剂量反应：
    #     0.5 −0.208 / 1.0 −0.187 / 1.5 −0.164 / 2.5 −0.038 / 3.0 −0.046 / 99.0(关闭) 0.000
    #   ⇒ 绕得越宽越差，方向明确、非尖峰；1.5（当前实盘语义）落在曲线劣质端。
    #   4 窗口共识（60×9/75×7/90×6/120×4）绕行 1.5 的 dSh：
    #     −0.083 / −0.063 / −0.164 / −0.038 → **4/4 全负**，无符号翻转。
    # 【★ 2026-09-23 21:20 OWNER 授权启用（False → True）】
    #   用户授权「增设缺陷修复通道」后，本项目晋升闸门由单通道改为双通道：
    #     --track alpha （默认）：闸门② 要求最差窗口 dSh ≥ +0.10（防 alpha churn）
    #     --track defect（缺陷修复）：闸门② 放宽为「方向不劣化」
    #         —— 均值 dSh ≥ 0 且最差窗口 ≥ −0.05，其余闸门③④⑤⑥⑦⑧ 照旧。
    #   本项以「实盘现状 G1（绕闸门）」为**反向对照**重跑：
    #     python strategy/_evolve_wf.py --mode consensus --track defect \
    #            --baseline "G1_轮动绕闸门_1.5"
    #   结果（dSh 即修复收益）：60×9 +0.083 / 75×7 +0.063 / 90×6 +0.164 / 120×4 +0.038
    #     → 均值 **+0.087 ≥ 0** ✓、最差 **+0.038 ≥ −0.05** ✓ ⇒ 闸门② PASS
    #     ①IS↑ PASS（ret 128.6→159.0%、Sh 1.10→1.25）；③ 6.3% PASS；
    #     ④ MDD −9.24% PASS；⑤ 最差折 −5.1% PASS；⑥ tests 238/0 PASS。
    #   **缺陷修复通道全部闸门通过 ⇒ 启用。**
    # 【不会导致空仓】闸门只拦截「trend_up=False 且 bias < 2.0」的弱候选；
    #   trend_up=True 的候选仍放行（今日 688012 即以 [bias=0.40 trend_up=True] 买入）。
    #   而 688008 那种 bias=−0.30 trend_up=False 的买入将被拒绝。
    # 【可逆 + 热读】置回 False 即可完全恢复；**无需重启引擎**
    #   （_rotation_daily_gate_ok 每次热读 STRATEGY_PARAMS），下一轮动评估周期生效。
...
# ============================================================
RISK_PARAMS = {
    "daily_loss_limit_pct": -0.03,      # 日内累计亏损 -3% 暂停新开仓（不平老仓）
    "daily_loss_limit_abs": 5000,       # 日内累计亏损 ¥5000 暂停新开仓
    # 【2026-09-16 优化】日内硬止损+强平阈值：当日账户亏损（相对开盘资产）达该比例
    #   即暂停新开仓并**强平全部可卖持仓**（不只停牌）。直接对冲隔夜跳空/盘中崩盘——
    #   此前 daily_loss_limit 只停牌不平仓，老仓完整吃了 9-16 的 -16% 隔夜缺口。
    #   -0.06 给趋势股正常波动留缓冲，深跌时果断脱手。
    "daily_stop_flatten_pct": -0.06,    # 当日账户亏损 <= -6% → 强平全部可卖持仓
    ...
    # 【2026-09-16 优化】原 -0.25 断路器低于回测历史最大回撤 -19.85%，实盘跌到 -19.6%
    #   都未触发，形同虚设。收紧到 -0.20：仍高于回测 MDD、不破坏已验证回测基线，
    #   但实盘深跌时更早介入。dd_recover_days 同步 5→3（已更深，恢复窗口收紧防长期踏空）。
    "max_drawdown_pct": -0.20,
    "dd_recover_days": 3,               # 回撤熔断后冷却 N 日自动恢复（重置风险基线）
    ...
    "max_order_amount": 180000,         # 单笔金额上限（与 max_position_amount 对齐，2026-09-16 收紧去风险）
}
...
# ============================================================
# A 股 T+1 交易规则（2026-09-14 新增，硬约束）
# ============================================================
# A 股实行 T+1 交收：当日买入的股票当日不可卖出，须到下一交易日方可卖出。
# 原 live/paper 引擎的日内破位/止损/轮动退出允许「同日买入即卖出」，会生成
# 实盘根本无法成交的 round-trip（如 2026-09-08 300394 于 10:21:14 买入、
# 10:21:17 即被「趋势破位」卖出）。此处统一在卖出唯一汇聚点 _handle_sell 拦截
# 「当日买入」的卖出，覆盖破位/止损/日线兜底/轮动换出/regime 强平全部路径。
# 默认开启（True）；仅当交易非 A 股（如可 T+0 的 ETF/转债/美股）时置 False。
T1_RESTRICTION = True

# ============================================================
# 建仓保护期（2026-09-14 新增，叠加于 T+1 之上）
# ============================================================
# 背景：T+1 已堵住「当日买入当日卖出」（如 300394 同日 round-trip）。但轮动换入的
# 候选在「首个可卖交易日」开盘即面临分钟级破位/止损退出，若早盘被正常波动扫到，
# 等于「换入即被误伤」。建仓保护期在 T+1 基础上，把不可卖窗口延伸到首个可卖交易日的
# 开盘后 ENTRY_PROTECT_MINUTES 分钟——给新仓（尤其轮动换入）一段不被早期波动止损的
# 缓冲，避免被分钟级 noise 误杀。
# 语义：pos.open_date 之后第一个交易日 09:30 起算，N 分钟内不触发任何退出（破位/止损/
# 日线兜底/轮动换出/regime 强平均经 _handle_sell 单点拦截）；N<=0 关闭。
# 默认值 30 分钟为保守缓冲；可据回测/实盘调参（0=关，或上调到 60 等）。
ENTRY_PROTECT_MINUTES = 30
