...
STRATEGY_PARAMS = {
    ...
    # 【闸门裁决】关闭绕行 = 4 窗口均值 dSh **+0.087**、最差窗口 **+0.038**、
    #   规范 wf(90×6) **+0.164**。按本项目既有多窗口口径（均值 ≥ +0.10）判定
    #   **未达闸门 ② ⇒ REJECTED（不启用）**；若按规范 wf 单窗口口径则达标。
    #   差异源于 120×4 长折对该效应不敏感（+0.038）。
    # 【可逆 + 热读】置 True/False 均**无需重启引擎**（_rotation_daily_gate_ok 每次
    #   热读 STRATEGY_PARAMS），下一个轮动评估周期（120s 节流）即生效。
    "rotation_require_daily_gate": False,
}
