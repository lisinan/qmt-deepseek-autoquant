...
STRATEGY_PARAMS = {
    ...
    "max_positions": 5,
    # 板块轮动 / 弱换强（2026-09-07 新增，默认关闭）：
    #   根因：策略「集中最强5只+趋势骑行+仅止损/破位退出」在满仓时无法参与新热板块
    #   （如 09-07 光模块踏空：5 槽被非光模块水下老仓占满，51% 现金干躺）。
    #   机制：组合已满时，若某热板块候选（LLM rerank top / 板块推荐池 top）的日线评分
    #   显著高于最弱持仓 ≥ rotation_min_score_gap，则换出最弱持仓、换入该候选。
    #   默认 enable_rotation=False → 当前「集中5只+趋势骑行」行为**完全不变**；
    #   确认开启后翻 True 即生效。rotation_cooldown_sec 防抖动（避免来回换仓）。
    #   ★ 2026-09-07 用户确认开启：板块轮动/弱换强正式生效。
    "enable_rotation": True,
    "rotation_min_score_gap": 2.0,   # 候选评分 − 最弱持仓评分 超此值才换仓
    "rotation_cooldown_sec": 1800,   # 两次轮动最小间隔（秒），防抖动
