...
class DailyContext:
    ...
    def trend_broken(self, code: str, exit_ma: int = 60) -> bool:
        ...
        【2026-09-22 AM-EVOLVE 证伪留痕】曾实现「破位需连续 N 日确认」
        （anti-whipsaw）以规避均线附近反复假破位的双向成本。90×6 OOS 网格
        结果**单调恶化**：N=2 −0.046 / 3 −0.077 / 4 −0.103 / 5 −0.125 /
        7 −0.197（N=1 精确复现基线，可逆性已验证）。结论：当前二值破位
        已是最优退出时点，并不存在值得修复的假破位损耗；延迟退出只会单边
        放大回撤。**已撤回实现，勿重试。**
        """
        with self._lock:
